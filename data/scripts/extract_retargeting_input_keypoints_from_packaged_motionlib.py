...
Supports SMPL and SOMA skeleton formats.
...
```bash
python data/scripts/extract_retargeting_input_keypoints_from_packaged_motionlib.py /path/to/your/packaged_motion_lib.pt --skeleton-format smpl --output-path /path/to/output_dir
```

## Parameters

- `packaged_motion_lib_file`: Path to the .pt file containing the packaged MotionLib data.
- `--skeleton-format`: Skeleton format to use: 'smpl' or 'soma' (default: smpl).
...
def main(
    ...
    # before the .pt file was created.
        ...
        try:
            ...
            if all_body_positions_current_motion.shape[1] != kinematic_info.num_bodies:
                print(
                    ...
                    f"This script assumes the .pt file stores data aligned with the selected skeleton. Skipping."
                )
                continue
