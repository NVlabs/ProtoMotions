...
def asset_path(*parts: Union[str, os.PathLike], must_exist: bool = True) -> Path:
    ...
    if must_exist and not path.exists():
        raise FileNotFoundError(
            ...
            "USD asset tree (IsaacLab/IsaacSim) and the SMPL/SMPL-H body-model "
            "assets, which carry their own licence terms. Use a Git LFS source "
            f"checkout or point {ASSET_ROOT_ENV} at a complete asset tree."
        )
    return path
