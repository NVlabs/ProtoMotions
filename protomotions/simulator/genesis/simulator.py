...
class GenesisSimulator(Simulator):
    ...
    def __init__(
        ...
        # Genesis does not support spawning objects - ensure scene_lib is empty
        assert (
            scene_lib.num_scenes() == 0
        ), "Genesis does not support spawning objects in the scene"
    ...
    def _create_simulation(self) -> None:
        ...
        self._scene = gs.Scene(
            sim_options=gs.options.SimOptions(
                dt=1.0 / self.config.sim.fps, substeps=self.config.sim.substeps
            ),
    ...
    def _create_envs(self) -> None:
        ...
        if asset_file_ext == ".xml":
            ...
            self._robot = self._scene.add_entity(
                gs.morphs.URDF(
                    ...
                    links_to_keep=self._body_names,
                ),
                visualize_contact=False,
            )
        ...
        for joint in self._robot.joints:
            if joint.name in self._dof_names:
        ...
        self._genesis_dof_indices = torch.tensor(
            self._genesis_dof_indices, device=self.device
        )
    ...
    def _get_sim_body_ordering(self) -> SimBodyOrdering:
        ...
        for joint in self._robot.joints:
            if joint.name in self._dof_names:
                common_dof_idx = self._dof_names.index(joint.name)
                if type(joint.dof_idx_local) is list:
                    for dof_idx_local in joint.dof_idx_local:
                        dof_offset = dof_idx_local - joint.dof_idx_local[0]
                        dof_names.append(self._dof_names[common_dof_idx + dof_offset])
                else:
    ...
    def _setup_markers(
        ...
        for marker_name, markers_cfg in visualization_markers.items():
            ...
            for marker_size, indices in scale_dict.items():
                scale_dict[marker_size] = torch.tensor(
    ...
    def _add_terrain(self) -> None:
        ...
        if (
            self.terrain.config.terrain_proportions[:-1].sum() == 0
            ...
            self._genesis_terrain = self._scene.add_entity(gs.morphs.Plane())
        else:
    ...
    def _set_simulator_env_state(
        ...
        self._robot.set_pos(new_states.root_pos, zero_velocity=False, envs_idx=env_ids)
        self._robot.set_quat(new_states.root_rot, zero_velocity=False, envs_idx=env_ids)
    ...
    def _get_simulator_dof_limits_for_verification(
        ...
        """
        Retrieve DOF limits from Genesis's internal API for verification purposes only.

        Returns:
            Tuple[torch.Tensor, torch.Tensor]: A tuple of (lower_limits, upper_limits)
                                              in Genesis's DOF ordering.
        """
        # Return the limits that were extracted during environment creation
        dof_limits_lower = []
        dof_limits_upper = []
        for joint in self._robot.joints:
            if joint.name in self._dof_names:
        ...
        return (
            torch.tensor(dof_limits_lower, device=self.device),
            torch.tensor(dof_limits_upper, device=self.device),
        )

    # ===== Group 5: Control & Computation Methods =====
    def _apply_simulator_pd_targets(self, pd_targets: torch.Tensor) -> None:
        """Applies PD position targets using Genesis's internal PD controller."""
        self._robot.control_dofs_position(pd_targets, self._genesis_dof_indices)
        ...
        for marker_name, markers_state_item in markers_state.items():
            ...
            if marker_dict.marker_type == "sphere":
                ...
                for marker_size, indices in marker_dict.scale.items():
                    ...
                    group_marker_pos = marker_pos_all[
                        :, indices
                    ]  # indices list for this scale group
                ...
                base_dir = torch.tensor(
                    [1.0, 0.0, 0.0], device=self.device, dtype=marker_quat.dtype
                )
                ...
                for marker_size, indices in marker_dict.scale.items():
                    ...
                    group_marker_pos = (
                        marker_pos_all[:, indices].view(-1, 3).cpu().numpy()
                    )  # positions for this group
                    group_marker_quat = marker_quat[
                        :, indices
                    ]  # quaternions for this group
