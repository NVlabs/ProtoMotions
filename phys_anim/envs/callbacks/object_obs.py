...
from phys_anim.envs.humanoid.humanoid_utils import (
    compute_relative_object_contact_bodies_jit,
...
class ObjectObs(BaseCallback):
    def __init__(self, config, env):
        ...
        self.object_pointclouds = torch.zeros(
            self.env.num_envs,
            self.env.max_objects_per_scene,
        ...
        self.object_pointclouds_obs = (
            torch.zeros(
                self.env.num_envs,
                self.env.max_objects_per_scene * self.config.num_features_per_object,
                device=self.env.device,
            )
            + 1000.0
        )
    ...
    def compute_observations(self, env_ids):
        ...
        if active_scenes.any():
            ...
            if self.env.contact_body_ids.numel() > 0:
                ...
                root_rot_inv = self.env.self_obs_cb.root_rot_inv[env_ids]
                ...
                relative_contact_bodies = torch_utils.quat_rotate(
                    root_rot_inv_expanded,
                    contact_bodies.reshape(-1, 3),
                    self.env.w_last,
                ).reshape(num_envs, self.env.contact_body_ids.shape[0], 3)

                if self.config.contacts_relative_to_object_root:
                    object_root_states = self.get_object_root_states()[
                        self.env.env_id_to_object_ids[env_ids].flatten(), :3
                    ].view(num_envs, -1, 1, 3)
                    target_points_for_contact = object_root_states
                else:
                    target_points_for_contact = relative_object_pointclouds

                contact_bodies_obs = compute_relative_object_contact_bodies_jit(
                ...
                object_pointclouds_obs = torch.cat(
                    [
                        relative_object_pointclouds.view(
                            num_envs, self.env.max_objects_per_scene, -1
                        ),
                        object_bounding_box,
            ...
            if self.config.use_object_identity:
                object_identity_one_hot = F.one_hot(
                    self.env.object_identity[env_ids],
                    num_classes=self.env.max_objects_per_scene,
                )
                object_pointclouds_obs = torch.cat(
                    [object_pointclouds_obs, object_identity_one_hot], dim=-1
                )
    ...
    def get_relative_object_pointclouds(self, env_ids, object_pointclouds):
        ...
        root_states = (
            self.env.get_humanoid_root_states()[env_ids].clone().view(num_envs, -1)
        )
        ...
        return pointclouds.view(
            num_envs, self.env.max_objects_per_scene, -1, 3
    ...
    def get_object_bounding_box_obs(self, scene_env_ids):
        ...
        max_objects = self.env.max_objects_per_scene

        root_states = self.env.get_humanoid_root_states()[scene_env_ids].view(
            num_scene_envs, -1
        )
        root_pos = root_states[:, :3]
        root_pos[:, -1] -= self.env.terrain_obs_cb.ground_heights[scene_env_ids].view(
            -1
        )
        ...
        expanded_root_pos = (
            root_pos.unsqueeze(1).expand(num_scene_envs, max_objects, 3).reshape(-1, 3)
        )
        expanded_root_quat = (
            root_quat.unsqueeze(1).expand(num_scene_envs, max_objects, 4).reshape(-1, 4)
        )
        ...
        bounding_box_obs = get_object_bounding_box_obs_jit(
            ...
            object_root_states_offsets=self.env.object_root_states_offsets,
