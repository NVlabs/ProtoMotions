...
"""RLFT a PEFT-adapted GPC prior for random target reaching.

The actor config declares the task-side PEFT conditioning network through
``actor.in_keys`` and ``actor.peft.model``. The frozen prior context is loaded
...
def env_config(robot_cfg: RobotConfig, args: argparse.Namespace) -> EnvConfig:
    ...
    return EnvConfig(
        ...
        reward_components={
            "target": target_reward_factory(weight=1.0, pos_err_scale=0.42),
        },
