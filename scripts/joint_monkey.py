...
class AssetDesc:
    def __init__(self, file_name, asset_type, fixed_base=False):
...
asset_descriptors = [
    AssetDesc("h1_2.xml", "mjcf", False),
    AssetDesc("g1.xml", "mjcf", False),
    AssetDesc("smplx_humanoid.xml", "mjcf", False),
    AssetDesc("rigv1_humanoid.xml", "mjcf", False),
]
...
print(f"Using physics engine: {args.physics_engine}")
...
# load asset
asset_root = "protomotions/data/assets/" + asset_descriptors[args.asset_id].asset_type
...
while not gym.query_viewer_has_closed(viewer):
    ...
    for i in range(num_envs):
        ...
        if args.show_axis:
            # draw lines for all DOFs
            for dof_idx in range(num_dofs):
                # get the DOF frame (origin and axis)
                dof_handle = gym.get_actor_dof_handle(
                    envs[i], actor_handles[i], dof_idx
                )
                frame = gym.get_dof_frame(envs[i], dof_handle)

                # draw a line from DOF origin along the DOF axis
                p1 = frame.origin
                p2 = frame.origin + frame.axis

                # print("DOF %d" % dof_idx)
                # print("  Name:     '%s'" % dof_names[dof_idx])
                # print(f"p1: {p1}, p2: {p2}")

                color_map = {
                    "_x": gymapi.Vec3(1.0, 0.0, 0.0),
                    "_y": gymapi.Vec3(0.0, 1.0, 0.0),
                    "_z": gymapi.Vec3(0.0, 0.0, 1.0),
                }
                color = color_map[dof_names[dof_idx][-2:]]
