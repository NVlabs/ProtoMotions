...
#
# Convenience script to retarget SOMA motions to a robot (G1 or H1_2)
#
# IMPORTANT: ProtoMotions and PyRoki require separate Python environments.
# You must provide paths to both Python interpreters.
#
# Usage: ./scripts/retarget_soma_to_robot.sh <proto_python> <pyroki_python> <soma_pt_file> <robot_type> [skip_freq] [--clean]
#
# Example:
#   ./scripts/retarget_soma_to_robot.sh \
#       ~/miniconda3/envs/protomotions/bin/python \
#       ~/miniconda3/envs/pyroki/bin/python \
#       /path/to/soma.pt g1 15 --clean
#
# Arguments:
...
#   soma_pt_file:  Path to packaged SOMA MotionLib .pt file (outputs saved in same directory)
...
# Parse arguments
if [ $# -lt 4 ]; then
    echo "Usage: $0 <proto_python> <pyroki_python> <soma_pt_file> <robot_type> [skip_freq] [--clean]"
    echo ""
    echo "Arguments:"
    ...
    echo "  soma_pt_file   Path to packaged SOMA MotionLib .pt file (outputs saved in same dir)"
    ...
    echo ""
    echo "Example:"
    echo "  $0 ~/miniconda3/envs/protomotions/bin/python ~/miniconda3/envs/pyroki/bin/python /data/soma.pt g1 15 --clean"
    exit 1
fi
...
if [ ! -f "$SOMA_PT_FILE" ]; then
    echo "Error: SOMA .pt file not found: $SOMA_PT_FILE"
    exit 1
fi

# Output directories are in the same location as input
OUTPUT_DIR="$(dirname "$SOMA_PT_FILE")"
...
echo ""
echo "[Step 3/5] Extracting foot contact labels from source SOMA motions..."
if [ "$ROBOT_TYPE" == "g1" ]; then
    $PYROKI_PYTHON pyroki/batch_retarget_to_g1_from_keypoints.py \
        --subsample-factor 1 \
        --keypoints-folder-path "$KEYPOINTS_DIR" \
